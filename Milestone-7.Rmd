...
```{r}
fin_seg_sub_p <- pdata.frame(fin_seg_sub, index =c("geo_id2","year"))

table(index(fin_seg_sub_p), useNA = "ifany")

plm_1 <- plm(dgepercap_cpi ~ H_citytract_NHW_i + diversityinterp + pctblkpopinterp + pctasianpopinterp + pctlatinopopinterp + medinc_cpi + pctlocalgovworker_100 + pctrentersinterp + pctover65 + pctcollegegradinterp + logpop, data = fin_seg_sub_p, model = "within", effect = "twoways")
