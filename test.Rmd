...
aggr_plot <- aggr(rp_aggr, bars = TRUE, col=c('navyblue','red'), numbers=TRUE, sortVars=TRUE, cex.axis=.5, gap=1, ylab=c("Histogram of missing data","Pattern"))
...
densityplot(imp_1_fs)
densityplot( x=imp_1_fs , data= ~ H_citytract_NHW_i + chng5pctblk + 
       chng5pctlatino + chng5pctasian + ideology_fill)
```
...
fin_dge <-fin %>% filter(dgepercap_cpi != 0)
fin_dge_5 <-fin_5 %>% filter(dgepercap_cpi != 0)
...
nrow(fin_dge) - nrow(fin_dge_5)

length(setdiff(fin_dge$geo_id2, fin_dge_5$geo_id2))
